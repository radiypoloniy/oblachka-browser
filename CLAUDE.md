...
- Граф-воркспейс (исследовательский холст, режим `'graph'` новой вкладки).
  ...
  они всегда поверх React-слоя. Веб-приложения приедут своим типом узла с
  панелью 1:1 (срез 3).
...
модалку блокнота, и карточки узлов) — СДЕЛАН. Дальше: 3 — узел
веб-приложения с передачей **через руку человека** (граф готовит промпт, кнопка
вставляет, отправляет пользователь, кнопка забирает ответ: полная автоматизация
чужого веб-UI отвергнута сознательно — гниющие селекторы, антибот и ToS, цена
ошибки — бан аккаунта ПОЛЬЗОВАТЕЛЯ); 4 — поиск и фактчек; 5 — поглощение
