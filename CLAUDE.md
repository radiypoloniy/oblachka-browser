...
- Граф-воркспейс (исследовательский холст, режим `'graph'` новой вкладки).
  ...
  открывается 1:1 в отдельной панели, ровно один за раз.
  `electron/GraphWebAppManager.ts` — вью чужого AI-сайта (sandbox +
  contextIsolation, БЕЗ preload, как `WebAppManager`), панель меряет «дырку» и
  шлёт координаты, main кладёт вью. `electron/graphWebApps.ts` — профили сайтов
