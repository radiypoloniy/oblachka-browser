...
- `electron/WindowRegistry.ts` — реестр окон (в работе, многооконность по срезам).
  ...
  сайдбара за край окна, запас 48 px) отдаёт ЖИВУЮ `WebContentsView`
  (`TabManager.detachTabForMove`/`adoptTab`), а не открывает адрес заново: у
  страницы остаются история «назад», прокрутка и введённое в форму. ⚠️ Слушатели
  прежнего менеджера снять нечем (`removeAllListeners` снёс бы и чужие — видео,
  PiP, индексация), поэтому каждый обработчик в `wirePageEvents` начинается с
  проверки `mine()`: ушедшую вкладку обслуживает уже новый владелец. Не переносятся
  хаб, псевдо-вкладки, спящие, закреплённые и участники split.
