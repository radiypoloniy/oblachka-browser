...
- `electron/WindowRegistry.ts` — реестр окон (в работе, многооконность по срезам).
  ...
  Оверлеи переводятся на «своё у каждого окна» по одному: `FindBarManager` и
  `SuggestDropdownManager` — уже (состояние в карте по id окна, окно ищется по
  вью-отправителю, потому что `BrowserWindow.fromWebContents` для дочерней вью
  отдаёт null). Осталось: поповеры паролей/автозаполнения, точка входа (Ctrl+N,
  ПКМ по вкладке, вытаскивание вкладки из сайдбара) и лёгкий режим отрисовки хрома.
