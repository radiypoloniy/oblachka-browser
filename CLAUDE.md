...
- `electron/WindowRegistry.ts` — реестр окон (в работе, многооконность по срезам).
  ...
  ⚠️ **Зоны дропа поверх страницы** (`zoneAt` в `Sidebar.tsx`): края контента по 35%
  ширины — разделить экран, середина — новое окно. Заведены потому, что у окна во
  весь экран «снаружи» нет вовсе, и жест не должен зависеть от границы окна. Chrome
  обходится без зон (полоса вкладок горизонтальная и тонкая: «вне полосы» = «новое
  окно»), но у нас полоса вертикальная и вне её — страница, уже занятая разделением
  экрана; браузеры с вертикальными вкладками и сплитом (Zen, Arc, Vivaldi) решают
  это ровно зонами. Подпись во время перетаскивания — не украшение: у одного дропа
  два исхода, и человек должен видеть, какой получит, до отпускания.
