...
| **Стадия** | Прод-сборка работает целиком: установщик, автообновление, перенос данных из Chrome/Edge/Яндекса, типизированный IPC-контракт и растущий набор автоматических проверок |
...
браузером по умолчанию, автообновление через `electron-updater`. Интерфейс новых профилей —
английский, у существующих остаётся русский.
...
**Дисциплина решений.** В `CLAUDE.md` — записанные причины: почему эмбеддинги удалены из проекта
целиком, почему группировка вкладок делается двумя прогонами модели плюс арбитражем обычным
кодом, почему координаты плиток на рабочем столе хранятся, а не вычисляются, почему разрешения
сайтов не могут быть двухпозиционными. Каждый пункт оплачен замером или сломанной функцией.
Это и есть скорость: следующий шаг не переоткрывает предыдущий.

---

## Темп разработки

Сотни коммитов и десятки тысяч строк на TypeScript (главный процесс + интерфейс) за недели,
а не за год. Строго типизированный код, единый контракт IPC между процессами, дизайн-система
на токенах.
...
**3. Размер.** Установщик весит сотни мегабайт, установленное приложение — ещё больше, и львиная
доля этого веса приходится на бэкенды локального ИИ под CPU, CUDA и Vulkan: одна и та же
программа должна запуститься и на видеокарте NVIDIA, и на встроенной графике. Сама модель
качается отдельно (3–6 ГБ) и только по решению пользователя. Для домашнего интернета терпимо,
но это фильтр на первом шаге.
...
**5. Проверки держат чистую логику, а не каждую гонку Electron.** `npm test` гоняет большой набор
проверок чистой логики и IPC-контракта. Гонки жизненного цикла вью и живое поведение Chromium
по-прежнему ловятся стендами и глазом. Этот зазор остаётся.
...
привязка к циклу обновлений Chromium. Осознанный размен: без него продукт такого объёма за
недели не существовал бы.
...
- [CONTRIBUTING.md](CONTRIBUTING.md) — как собрать, что обязано быть зелёным и как устроены
  сторожа, без которых правку не примут.
...
Интерфейс новых профилей — английский. Большая часть этих файлов пока на русском.
