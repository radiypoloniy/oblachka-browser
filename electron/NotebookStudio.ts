  ...
  switch (kind) {
    ...
    case 'mindmap':
      ...
      return 'По приведённым ниже источникам построй иерархический план для майндкарты в формате '
        ...
        + context;
    case 'infographic':
      // Модель выдаёт декларативный синтаксис AntV Infographic, рендерит его движок
      // @antv/infographic в renderer.
      //
      // ⚠️ Имя шаблона обязано существовать в реестре библиотеки. Раньше здесь стоял
      // выдуманный 'list-column': такого шаблона нет, а resolveTemplateKey при промахе
      // молча берёт ближайший по написанию (findClosestTemplateKey) — попадали в
      // 'list-column-done-list', список галочек. А DoneList.tsx рисует
      // `datum.label || datum.desc`, то есть при наличии label описание выбрасывается
      // совсем. Отсюда и брались инфографики из одних заголовков без единого факта.
      //
      // 'list-grid-compact-card' — настоящий шаблон, его CompactCard рисует все четыре поля
      // (icon, label, value, desc). Поэтому просим ещё и value: инфографика без чисел
      // вырождается в оглавление, а число — ровно то, ради чего на неё смотрят.
      return 'По приведённым ниже источникам построй инфографику в синтаксисе AntV Infographic. '
        ...
        + 'data\n'
        + '  title Короткий заголовок всей инфографики\n'
        + '  items\n'
        + '    - label Суть пункта (2–4 слова)\n'
        + '      value 34%\n'
        + '      desc Факт одним предложением (до 14 слов)\n'
        + '    - label Второй пункт\n'
        + '      value 11К\n'
        + '      desc Факт одним предложением\n\n'
        + 'Сделай 4–6 пунктов. В value ставь ГЛАВНОЕ число пункта — долю, количество, сумму '
        + 'или год. ⚠️ value обязано быть ОЧЕНЬ коротким: без пробелов и НЕ ДЛИННЕЕ 4 символов, '
        + 'иначе оно не влезает в карточку и наезжает на текст. Крупные числа сокращай: '
        + '10 700 → «11К», 4,6 млн → «4.6М», 8 100 → «8.1К». Допустимо: «34%», «2.8%», «2025». '
        + 'Точное значение и единицы пиши словами в desc — там места хватает. Если числа у '
        + 'пункта нет, строку value пропусти совсем. desc заполняй ВСЕГДА, это главный смысл '
        + 'пункта. Пиши по-русски. Опирайся ТОЛЬКО на источники, ничего не выдумывай.\n\n'
        + context;
    case 'quiz':
