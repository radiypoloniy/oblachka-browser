...
function writeInUiLanguage(): string {
  return uiLanguage() === 'en' ? 'Write all user-facing text in English. ' : 'Пиши по-русски. ';
}
...
function buildPrompt(kind: StudioKind, context: string): string | null {
  switch (kind) {
    case 'summary':
      return writeInUiLanguage() + 'По приведённым ниже источникам сделай краткое структурированное саммари в '
    ...
    case 'mindmap':
      ...
      return writeInUiLanguage() + 'По приведённым ниже источникам построй иерархический план для майндкарты в формате '
    ...
    case 'infographic':
      ...
      return writeInUiLanguage() + 'По приведённым ниже источникам построй инфографику — 4–6 карточек с фактами. '
        ...
        + 'предложение до 14 слов, заполняй всегда. Опирайся ТОЛЬКО на '
    ...
    case 'quiz':
      ...
      return writeInUiLanguage() + 'По приведённым ниже источникам составь тест из 4–5 вопросов с вариантами ответа. '
        + 'Ответь СТРОГО валидным JSON без пояснений и без ограждений ```. Формат:\n'
        ...
        + '4 варианта. Опирайся ТОЛЬКО на источники, ничего не выдумывай.\n\n'
        + context;
    default:
      return null;
  }
