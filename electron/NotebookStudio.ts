...
function buildDocPrompt(context: string): string {
  const rules = [
    ...
    'Начни с блока cover (заголовок документа и одна строка подписи). Дальше 14–20 блоков,',
    'разбитых на 4–6 разделов: каждый раздел — heading плюс один-два text.',
    'heading — заголовок раздела; text — абзац 3–5 предложений, полными фразами, а не',
    'перечислением; quote — одна мысль, которую стоит запомнить; list — 3–6 коротких пунктов;',
    'metrics — 2–4 числа с подписями (label — что это, value — само число с единицей);',
    'table — пары «параметр → значение»; compare — пары для сравнения; sources — перечень',
    'источников (label — название, value — адрес).',
