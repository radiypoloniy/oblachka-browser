...
async function runPromptQueued(prompt: string, maxTokens: number, onChunk?: (text: string) => void): Promise<{ out: string; tokens: number; stopReason: string }> {
  ...
  // ⚠️ Ответ модели НЕ воспроизводится от прогона к прогону, хотя temperature по умолчанию 0
  // (жадная выборка, один вход обязан давать один выход). Замерено отдельным опытом:
  //  • KV-кэш перед каждым прогоном ПУСТ (`nextTokenIndex` = 0 до и после очистки) — то есть
  //    остаточное состояние контекста ни при чём, и очистка ПЕРЕД прогоном ничего не меняет
  //    (пробовал, откачено — не надо чинить то, что не сломано);
  //  • квантованный KV-кэш ни при чём — с f16 картина та же;
  //  • первый прогон в процессе систематически отличается от последующих, а последующие между
  //    собой совпадают.
  // Механизм не установлен. Практический вывод, который из этого следует и уже соблюдается:
  // замер обязан идти с повторами и после прогрева (см. scripts/ai-bench.mjs), а ни одна фича не
  // вправе рассчитывать на воспроизводимость одного ответа — отсюда валидация ответа кодом и
  // подтверждение человеком там, где цена ошибки не нулевая.
...
/**
 * Загружена ли модель ПРЯМО СЕЙЧАС (или уже грузится).
 *
 * ⚠️ Нужна тем вызовам, которые человек НЕ заказывал явно. Замер на живой машине: холодная
 * загрузка 9B — 31 секунда и почти шесть гигабайт VRAM. Для нажатой кнопки перевода это честная
 * плата (человек её ждёт), а вот для подсказки в омнибоксе — нет: печатая фразу, никто не просил
 * поднимать модель. Такие фичи обязаны спрашивать здесь и молча пропускать ход, пока модель
 * холодная (см. TABS_SEARCH_SMART в main.ts).
 */
