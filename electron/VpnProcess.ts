...
//
// session.setProxy живёт в main.ts::applyVpnProxy, не здесь: этот модуль только поднимает
// процесс и проверяет, что локальный SOCKS-порт отвечает. Решение про трафик — у вызывающей
// стороны, по состоянию ('running' / 'error' / 'stopped').
//
...
/**
 * Убрать оперативный конфиг с диска (аудит 21.08).
 *
 * ⚠️ В файле лежат uuid/пароль сервера ОТКРЫТЫМ ТЕКСТОМ — иначе Xray его не прочитает. Пока
 * туннель работает, файл нужен; после остановки он не нужен НИКОМУ, а лежал до следующего
 * подключения. Уровень защиты был как у истории (учётка ОС), но у credential от чужого сервера
 * цена утечки другая: им можно пользоваться без нашего браузера вовсе.
 *
 * ⚠️ Стираем ПОСЛЕ выхода процесса, а не до: Xray держит файл открытым, и удаление на живом
 * процессе на Windows либо не пройдёт, либо оставит его без конфига в момент переподключения.
 *
 * ⚠️ Отсутствие файла — норма, а не ошибка: stop() зовётся и там, где старта не было.
 * Персистентная подписка при этом не трогается — она в отдельном зашифрованном хранилище
 * (VpnKeyStore.ts), и туннель после этого поднимается заново без единого вопроса человеку.
 */
function removeConfigFile(): void {
  try {
    ...
    if (code !== 'ENOENT') console.warn('[VPN] не удалось убрать конфиг:', code ?? (e as Error).message);
  }
}
