...
/**
 * Сырое вложение от модели: тип и байты в base64, как их отдал провайдер.
 *
 * ⚠️ Адаптер НЕ ходит на диск. Он знает форму чужого ответа и больше ничего; куда положить байты и
 * как назвать файл — решение этажом выше, одно на всех провайдеров. Иначе четыре адаптера завели бы
 * четыре слегка разных каталога с файлами.
 */
export interface RawFile { mime: string; base64: string }

export interface ChatResult {
  out: string;
  /**
   * Картинки, которые вернула модель.
   *
   * ⚠️ Есть далеко не у всех: Anthropic картинок не отдаёт вовсе, локальная Qwen тоже. Поле
   * необязательное именно поэтому — «пустой массив» соврал бы, что провайдер их поддерживает.
   */
  files?: RawFile[];
  history: unknown[];
  ms: number;
