...
function createWindow(role: WindowRole = 'main') {
  ...
  const win = new BrowserWindow({
    ...
    minWidth: 900,
    minHeight: 600,
    // Ровно --chrome-ground светлой темы (см. colors.css: --app-bg смешан с 11% чёрного), чтобы не
    // мигало белым и не было шва под шапкой. ⚠️ Раньше здесь стоял --app-bg; после того как землю
    // под хромом сделали темнее, литерал пришлось сдвинуть за ней — иначе первый кадр окна светлее
    // интерфейса, который на нём появится.
