...
/**
 * Сколько ждём человека, прежде чем ответить агенту словами.
 *
 * ⚠️ Было 60 секунд, стало 25, и это не «ускорение». Клиенты рвут вызов по своему таймауту, и
 * молчание дольше него агент видит как поломку сервера — а сказать ему есть что: карточка висит
 * и ждёт. Уходя ответом раньше, мы даём агенту шанс объяснить человеку, что от него нужно.
 * Карточка при этом НЕ снимается: подтвердивший через минуту всё равно будет услышан, а повтор
 * вызова пройдёт молча.
 */
 ...
 * Спросить человека.
 *
 * ⚠️ ИСХОДА ТРИ, А НЕ ДВА, и различать их обязательно. «Отказал» и «ещё не ответил» — для агента
 * противоположные новости: первая означает «даже не пробуй», вторая — «попроси человека нажать и
 * повтори». Пока оба отвечали одинаково, агент читал отказ и уходил в обход — живая жалоба
 * 04.09.2026: «MCP-вызов вкладки не проходит, открою напрямую», хотя карточка в это время висела
 * в окне браузера и ждала одного нажатия.
 *
 ...
 */
export type ConfirmOutcome = 'allowed' | 'refused' | 'waiting';

export async function confirmWrite(req: ConfirmRequest): Promise<ConfirmOutcome> {
  ...
  const timeout = new Promise<'waiting'>((resolve) => { setTimeout(() => resolve('waiting'), WAIT_MS); });
  return Promise.race([ask.then((yes): ConfirmOutcome => (yes ? 'allowed' : 'refused')), timeout]);
}
