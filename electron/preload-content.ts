...
// ─── СПОРНЫЕ ХОТКЕИ: страница вперёд браузера ───────────────────────────────
//
// ⚠️ ЗАЧЕМ. `before-input-event` в main срабатывает РАНЬШЕ страницы, и наш `preventDefault()`
// означает, что страница клавишу не увидит вообще. Для Ctrl+T или Ctrl+W это правильно — их у
// браузера не отнимают. Но Ctrl+D в Google Таблицах это «заполнить вниз», Ctrl+R — «заполнить
// вправо», Ctrl+E — выравнивание по центру, а Ctrl+F и Ctrl+H — их собственные поиск и замена,
// которые умеют больше нашего. Живая формулировка: «внутри гуглдоков хоткеи должны быть важнее,
// чем браузерные».
//
// ⚠️ БЕЗ СПИСКА САЙТОВ. Перечислять docs.google.com, notion.so, figma.com — заведомо неполный
// путь: завтра появится шестой редактор, и он снова будет ломаться. Здесь работает признак, а
// не имя: слушаем ВСПЛЫТИЕ на window, то есть уже ПОСЛЕ обработчиков страницы, и смотрим на
// `defaultPrevented`. Страница вызвала preventDefault — значит клавиша ей нужна, и браузер
// молчит. Не вызвала — отправляем действие в main. Это ровно та же модель, по которой ведёт
// себя Chrome со своими нерезервированными сочетаниями.
//
// ⚠️ `stopPropagation()` на странице тоже считается «страница забрала»: до нашего слушателя
// событие просто не дойдёт, и мы ничего не сделаем. Это верный исход, а не пропажа: если сайт
// глушит событие насмерть, он им пользуется.
//
// ⚠️ Аварийный выход остаётся ВСЕГДА: перезагрузка живёт ещё и на F5 / Ctrl+Shift+R, а они
// по-прежнему перехватываются в main и странице не отдаются. Без этого страница, съевшая
// Ctrl+R, оставила бы человека без клавиатурной перезагрузки.
//
// ⚠️ Канал зашит строкой — sandboxed preload не может require() относительные модули
// (см. разбор в шапке файла). Значение обязано совпадать с IPC.PAGE_HOTKEY.
const CH_PAGE_HOTKEY = 'page:hotkey';

/** Спорные сочетания: код клавиши → имя действия для main. Только БЕЗ Shift и БЕЗ Alt. */
const DISPUTED: Record<string, string> = {
  KeyF: 'find',      // у редакторов свой поиск, и он богаче нашего
  KeyE: 'quick',     // Таблицы/Документы: выравнивание по центру
  KeyD: 'bookmark',  // Таблицы: заполнить вниз
  KeyR: 'reload',    // Таблицы: заполнить вправо
  KeyH: 'history',   // Таблицы/Документы: замена
};

try {
  window.addEventListener('keydown', (e: KeyboardEvent) => {
    // Ctrl без Shift/Alt/Meta — ровно тот набор, что разбирает main.
    if (!e.ctrlKey || e.shiftKey || e.altKey || e.metaKey) return;
    if (e.repeat) return;                 // удержание не должно открывать поповер десять раз
    const action = DISPUTED[e.code];
    if (action === undefined) return;
    // Страница уже сказала, что клавиша её. Молчим.
    if (e.defaultPrevented) return;
    // Наша очередь: гасим родное поведение (у Ctrl+F это встроенный поиск Chromium) и отдаём
    // действие браузеру.
    e.preventDefault();
    try { ipcRenderer.send(CH_PAGE_HOTKEY, action); } catch { /* вью уже уничтожена */ }
  });
} catch {
  // Слушатель не встал — спорные хоткеи просто не работают на этой странице, браузинг цел.
}
