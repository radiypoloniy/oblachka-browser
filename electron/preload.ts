...
const api: OblakoApi = {
  ...
  setOmniboxEditing: (on: boolean) => ipcRenderer.send(IPC.OMNIBOX_SET_EDITING, on),
  ...
  getOmniboxResume: () => ipcRenderer.invoke(IPC.OMNIBOX_RESUME) as Promise<OmniboxResume>,
  // ⚠️ Дорогой вызов (текст живой страницы + сравнение со снимком) — Toolbar.tsx спрашивает раз на адрес, а не на каждое открытие панели.
  ...
  onWindowMaximized: (cb: (maximized: boolean) => void) => {
    ...
    ipcRenderer.on(IPC.WINDOW_MAXIMIZED, h);
    return () => { ipcRenderer.off(IPC.WINDOW_MAXIMIZED, h); };
  },
  ...
  createProfile: (name: string, color: string) => ipcRenderer.invoke(IPC.PROFILES_CREATE, name, color) as Promise<ProfilesState>,
  ...
  onGenWidgetProgress: (cb: (p: GenProgress) => void) => {
    const handler = (_e: unknown, p: GenProgress) => cb(p);
    ipcRenderer.on(IPC.DESKTOP_GEN_PROGRESS, handler);
    return () => ipcRenderer.removeListener(IPC.DESKTOP_GEN_PROGRESS, handler);
  },
  ...
  // Таймер стола: состояние держит main (electron/TimerService.ts), виджет его только показывает.
  getTimer: () => ipcRenderer.invoke(IPC.TIMER_GET) as Promise<TimerState>,
  setTimer: (next: Partial<TimerState>) => ipcRenderer.invoke(IPC.TIMER_SET, next) as Promise<TimerState>,
  onTimerChanged: (cb: (state: TimerState) => void) => {
    const handler = (_e: unknown, state: TimerState) => cb(state);
    ipcRenderer.on(IPC.TIMER_CHANGED, handler);
    return () => ipcRenderer.removeListener(IPC.TIMER_CHANGED, handler);
  },
  ...
  generateStudio: (kind: string, context: string, sources?: { title: string; url: string }[]) => ipcRenderer.invoke(IPC.NOTEBOOK_STUDIO_GEN, kind, context, sources) as Promise<{ ok: boolean; text?: string; error?: string }>,
  ...
  getPageLength: () => ipcRenderer.invoke(IPC.SETTINGS_GET_PAGE_LENGTH) as Promise<PageLength>,
  setPageLength: (v: PageLength) => ipcRenderer.invoke(IPC.SETTINGS_SET_PAGE_LENGTH, v) as Promise<void>,
  ...
  onMediaState: (cb: (state: MediaNowPlaying | null) => void) => {
    const handler = (_e: unknown, state: MediaNowPlaying | null) => cb(state);
    ipcRenderer.on(IPC.MEDIA_STATE_CHANGED, handler);
    return () => ipcRenderer.removeListener(IPC.MEDIA_STATE_CHANGED, handler);
  },
