...
export function createWindowTabManager(
  ...
  tabs = new TabManager(
    ...
    (url, title, wc) => {
      ...
      // Живая индексация текста: на визит. page-title-updated ниже может дать вторую попытку,
      // если на did-navigate заголовок ещё имя сайта («YouTube») — иначе страница навсегда
      // оставалась без чанка до рестарта. SPA-тики отсекает shared/historyIndex.ts.
      // wc — вкладка, которая реально навигировала, не обязательно активная.
      void indexVisit(visitHistory, url, title, wc, { trigger: 'navigate' }).catch((e: unknown) =>
    ...
    (url, title, wc) => {
      const titleHistory = historyFor(tabs?.profileOfWebContents(wc.id) ?? DEFAULT_PROFILE_ID);
      titleHistory.updateTitle(url, title);
      void indexVisit(titleHistory, url, title, wc, { trigger: 'title' }).catch((e: unknown) =>
        console.warn('[HistoryIndexer] неожиданная ошибка:', e),
      );
    },
  ...
  // Перед выгрузкой вкладки добрать текст, если навигация не успела. false — извлечение ещё
  // идёт, sleepTab оставит вкладку до следующего тика, а не убьёт вью под снимком.
  tabs.setOnBeforeSleep(async (url, title, wc) => {
