    ...
    if (wantsRealWindow) {
      return {
        ...
        overrideBrowserWindowOptions: {
          ...
          // ⚠️ partition здесь НЕ задаётся, и это НЕ упущение — не «чините». Дочернее окно с
          // живым window.opener у Chromium ОБЯЗАНО делить контекст с открывшей страницей, иначе
          // opener не работает, а он тут и есть весь смысл настоящего окна. Сессию попап
          // наследует сам: из приватной вкладки — приватную. Замерено, а не выведено —
          // scripts/popup-partition-drive.mjs (кука попапа видна внутри инкогнито и НЕ видна из
          // обычной вкладки). Это же снимает находку 8 аудита 23.08, стоявшую как «не
          // воспроизведена»: по коду она читалась дырой, по замеру её нет.
