...
const TABS = [
  ...
  // ⚠️ Живой случай с профиля: две новости об одном (погода в Москве) на РАЗНЫХ сайтах, причём у
  // одного из них открыто ещё несколько вкладок. Раньше пара разъезжалась — статью с «большого»
  // сайта забирала доменная группа, а вторая оставалась одна. Обе обязаны оказаться вместе.
  { slug: 'heat',    title: 'В Москву возвращается жара до 35 градусов',  host: SITE_HOST },
  { slug: 'storm',   title: 'Оранжевый уровень погодной опасности объявлен в столице', host: '127.0.0.11' },
];
...
async function suiteOrganize(chrome) {
  ...
  return {
    ...
    cases: [
      ...
      // Пара по смыслу с разных сайтов не должна разъезжаться из-за доменной группы.
      { name: 'две новости об одном не разорваны доменом', ok: together('heat', 'storm'), got: JSON.stringify(clusters), want: 'вместе', ms: 0, log },
