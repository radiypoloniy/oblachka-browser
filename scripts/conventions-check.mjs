...
//   3. акцент берётся токеном, а не литералом: иначе палитра его не переопределит.
{
  ...
  // (4) Цвет не появляется в НАСТОЯЩЕМ ХРОМЕ — тулбар, полоса вкладок, сайдбар.
  //
  // ⚠️ ПРАВИЛО ПЕРЕПИСАНО 23.08.2026 вместе с дизайн-системой 2.0, и вот почему. Раньше оно
  // запрещало цвет в НАСТРОЙКАХ, и формулировка звучала логично: «в настройках цвет значит
  // выбрано». Но настройки — не хром: там нет чужого сайта, с которым цвет мог бы спорить.
  // Запрет превратил все страницы приложения (настройки, история, загрузки) в серую массу из
  // строк кегля 14 — ровно та жалоба, с которой начался редизайн.
  //
  // Граница теперь проходит там, где она содержательна: ХРОМ — это рамка ВОКРУГ чужого сайта,
  // и она обязана молчать. СТРАНИЦА — самостоятельный экран приложения, ей цвет положен.
  const CHROME_FILES = [
  ...
  for (const f of CHROME_FILES) {
    ...
    codeLines(fs.readFileSync(f, 'utf8')).forEach((line, i) => {
      ...
      if (line.includes('var(--poster-')) {
        hits.push(rel(f) + ':' + (i + 1) + '  плакатный цвет в хроме — он только для страниц');
      }
    });
  }

  // (4б) Плакатный цвет ходит ПАРОЙ со своей краской. ⚠️ Не стилистика, а контраст: на мандарине,
  // горчице, лайме и небе чернила дают выше 7:1, а белый — ниже 3:1; на чае и страсти наоборот.
  // Строка, которая красит фон плакатным цветом и тут же ставит белый литерал, нечитаема на
  // половине набора — поэтому краска берётся токеном --on-poster-*.
  for (const f of walk(path.join(ROOT, 'src'), ['.tsx'])) {
    codeLines(fs.readFileSync(f, 'utf8')).forEach((line, i) => {
      if (/background[^;]*var\(--poster-/.test(line) && /color:\s*'#(fff|FFF|ffffff|FFFFFF)'/.test(line)) {
        hits.push(rel(f) + ':' + (i + 1) + '  белый литерал на плакатном цвете — нужен var(--on-poster-*)');
      }
    });
  }
