...
// ⚠️ Случаи ниже добавлены по итогам мутационного прогона (npm run mutate -- fileNameSafety).
// Проверки на длину были, но брали заведомо далёкие значения — 160 символов и 2. Из-за этого
// `>` спокойно менялось на `>=`, а 70 на 71, и прогон оставался зелёным: границу не проверял
// никто. Здесь именно граница, по обе стороны — иначе off-by-one в санитайзере имени файла
// пройдёт незамеченным до первого испорченного имени на диске.
console.log('\n— границы длины (мутационный прогон) —');
const name70 = 'А'.repeat(70);
check('имя ровно в потолок (70) проходит',
  sanitizeFileNameBase(name70, '.pdf'), name70);
check('имя на символ длиннее потолка (71) отбраковывается',
  sanitizeFileNameBase('А'.repeat(71), '.pdf'), null);
check('имя ровно в минимум (4) проходит',
  sanitizeFileNameBase('Отчё', '.pdf'), 'Отчё');
check('имя на символ короче минимума (3) отбраковывается',
  sanitizeFileNameBase('Отч', '.pdf'), null);
// DEL (127) идёт отдельным случаем от диапазона 0..31: в коде это отдельное сравнение, и без
// своего входа оно не проверялось ничем.
check('DEL (127) заменяется пробелом, как и остальные управляющие',
  sanitizeFileNameBase(`Счёт${String.fromCharCode(127)}№12`, '.pdf'),
  'Счёт №12');
