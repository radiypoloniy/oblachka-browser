...
// ── 5. лёгкое углубление синего ──────────────────────────────────────────────
// ⚠️ Дуотона здесь БОЛЬШЕ НЕТ, и это не упрощение. Он появился, когда исходник был бледным
// лавандово-РОЗОВЫМ: разделить облако и фон по насыщенности не выходило (оба почти белые), и вся
// картинка прогонялась через шкалу «глубокий синий … белый». Цветовой закон это чинило, но ценой
// объёма — иконка выходила плоской, и её отклонили.
//
// Теперь исходник приводится к синему ОТДЕЛЬНЫМ шагом (scripts/recolor-logo.mjs, сектор сиреневого
// складывается к голубому краю), и дуотон стал не нужен: своя мягкая растяжка логотипа остаётся
// целиком. Остаётся только слегка углубить синь — светлый круг иначе сливается со светлой панелью
// задач (та самая живая жалоба про контраст).
const DEEPEN = 0.86;   // множитель светлоты ФОНА; 1 — не трогать вовсе
for (let i = 0; i < OUT * OUT; i++) {
  ...
  const l = (mx + mn) / 510;
  // ⚠️ Переход ПЛАВНЫЙ, а не по порогу. Первая версия обрывала углубление на L > 0.9, и по краям
  // облаков шла крапчатая ступенька: соседние пиксели попадали по разные стороны порога.
  // smoothstep по светлоте: тёмный фон углубляется целиком, белое облако не трогается вовсе,
  // между ними — гладкая растяжка.
  const t = Math.max(0, Math.min(1, (l - 0.72) / (0.92 - 0.72)));
  const k = DEEPEN + (1 - DEEPEN) * (t * t * (3 - 2 * t));
