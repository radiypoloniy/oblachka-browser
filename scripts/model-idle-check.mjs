...
{
  // Случай-ловушка, ради которого заведён VRAM_OTHERS_MIN: 9B на карте с 8 ГБ сама оставляет
  // меньше 15 % свободными. Правило по одной доле выгружало бы модель сразу после загрузки.
  check('своя модель на забитой карте — не повод выгружаться',
    isVramTight({ total: 8 * GB, free: 0.9 * GB, freeAtLoad: 0.9 * GB }), false);

  // Та же карта, но пришла игра и забрала полтора гигабайта.
  check('чужая программа забрала полтора гигабайта — тесно',
    isVramTight({ total: 8 * GB, free: 0.4 * GB, freeAtLoad: 1.9 * GB }), true);

  // ⚠️ Просторная карта: чужой забрал даже больше, но свободного всё равно много.
