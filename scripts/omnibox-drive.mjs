...
/**
 * Дождаться, пока число строк перестанет меняться.
 *
 * ⚠️ Два одинаковых замера подряд, а не «подождать подольше»: фиксированная пауза либо мала на
 * медленной машине, либо тратит секунды на быстрой, и в обоих случаях врёт молча.
 */
async function settle(drop, tries = 24) {
  let prev = -1;
  let same = 0;
...
await withStand(async (ctx) => {
  ...
  // ⚠️ ЖДЁМ, ПОКА СПИСОК УСТАКАНИТСЯ, и это не перестраховка. Список собирается ДВАЖДЫ на одно
  // нажатие: сразу — история и вкладки, повторно — когда доедут живые подсказки поисковика (им
  // отведено до трёх секунд). Первая версия снимала число строк ОДИН раз и дальше считала его
  // неизменным — из-за чего прогон падал каждый раз по-разному: то шесть строк, то две, а
  // кольцевые проверки считали от разного числа. Выглядело это как «плавающий баг в омнибоксе», а
  // было двумя разными вещами сразу: настоящей поломкой (пересборка отбирала выбор у человека,
  // см. shared/omniboxSelection.ts) и вот этой слепотой драйвера.
  const first = await settle(drop);
  check('список собрался и устаканился', first.rows >= 2, `строк ${first.rows}`);
