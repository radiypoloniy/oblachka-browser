...
want('не выгружать модель', 'Локальная модель');
...
console.log('\n— целостность реестра —');
{
  // ⚠️ Проверяем ПОДПИСИ, а не адреса: раздел без блока (в «AI» это «Фактчек и веб-поиск»)
  // законно ведёт в корень секции. А вот две одинаковые подписи в выдаче означали бы выбор
  // без разницы.
  const seen = new Set();
