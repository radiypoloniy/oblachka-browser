...
//
// ⚠️ Снимок начала, не «сейчас». К 17.09.2026 те же узлы нарезаны (baseline): Toolbar 548 / 5
// эффектов / 13 IPC, App 602 / 4 эффекта / 24 IPC, createWindow 189, Sidebar 709, aiApps 654,
// TabManager ~3244. Актуальная таблица — docs/architecture-code.md §5.
//
...
const LIMITS = {
  ...
  // ⚠️ 40, а не 25: при 25 крупный блок пришлось бы дробить на пять хуков ради счётчика, а
  // пять хуков по три подписки читаются хуже одного по пятнадцати. Порог ловит «компонент стал
  // контейнером всего приложения» (Toolbar было 83, App 78; сейчас 13 и 24), а не диктует размер блока.
  ipc: 40,        // обращений window.oblako.* в файле
};
