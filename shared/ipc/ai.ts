...
export interface AiActivityState {
  ...
  chars: number;
  /** Считается на этой машине. ⚠️ Красит светодиод: цвет означает «текст никуда не улетает». */
  local: boolean;
  /** Сколько работ идёт всего. Больше одной бывает только с облаком — туда шлём параллельно. */
  count: number;
}
...
/**
 * Что ПРОСИТЬ у модели на каждой ступени.
 *
 * ⚠️ Главное, что здесь надо понять: maxTokens — это ПОТОЛОК, а не заказ. Он умеет только
 * обрезать и никогда не заставляет писать длиннее. Пока промпт был один на все ступени
 * («4–6 разделов, 2–3 абзаца»), модель писала свои примерно пять тысяч знаков при любом выборе,
 * и настройка не делала ничего — что и было замечено сразу: «не увидел разницы между короткой,
 * средней и длинной».
 *
 * ⚠️ Поэтому ступень меняет ПРОСЬБУ: сколько разделов, сколько абзацев в каждом и какой длины
 * предложения. Потолок остаётся страховкой от заноса, а не инструментом длины.
 */
export interface PageLengthSpec {
  sections: string;
  paras: string;
  sentences: string;
  /** Просить ли список и таблицу: на короткой они съедают место у текста. */
  extras: boolean;
}

export const PAGE_LENGTH_SPEC: Record<PageLength, PageLengthSpec> = {
  short:  { sections: '3–4', paras: 'один',      sentences: '3–4', extras: false },
  normal: { sections: '4–6', paras: 'один-два',  sentences: '4–6', extras: true },
  long:   { sections: '6–8', paras: 'два-три',   sentences: '5–7', extras: true },
};
