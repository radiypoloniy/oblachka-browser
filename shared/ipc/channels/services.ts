...
export const SERVICES = {
  ...
  MCP_CLIENT_INSTALL: 'mcp:client-install', // renderer → main: (clientId) -> McpInstallResult
  // Подключить программу задним числом, из журнала обращений. ⚠️ То же согласие человека, что и
  // на карточке, — просто данное позже: карточка появляется в окне браузера, куда человек в
  // момент вызова не смотрит (он в той самой программе, из которой спрашивает).
  MCP_APPROVE: 'mcp:approve', // renderer → main: (clientKey, label) -> McpServerState
