...
export interface McpClientInfo {
   ...
   */
  profileName?: string;
  /** Белый список сайтов. Пустой или отсутствует — программа видит любые. */
  domains?: string[];
...
export interface McpPromptRequest {
  ...
  client: string;
  /** Вопрос: «Открыть вкладку?». ⚠️ Именно вопрос — карточка спрашивает, а не сообщает. */
  title: string;
  /** Предмет вопроса: адрес, что произойдёт, перечень доступного. */
  detail: string;
