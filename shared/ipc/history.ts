...
export interface ImportTypeResult {
  ...
  skipped: number;
  unsupported?: number;
  // Профиль Firefox закрыт мастер-паролем, и присланный (или пустой) не подошёл. Отдельный признак,
  // а не unsupported: это не «перенести нельзя», а «спроси пароль и повтори» — единственный исход
  // импорта, который лечится действием человека, поэтому UI обязан отличать его от нуля записей.
