...
export interface OmniboxPanel {
  ...
  /**
   * Реранк ещё идёт: столько же пустых карточек, сколько потом будет настоящих.
   * FTS в `related` не кладём — мелькание чужих заголовков хуже секунды ожидания.
   */
...
/**
 * Превью файла для списка загрузок. `thumb` — кадр картинки (лунка), `shell` — значок
 * Проводника без подложки: у глифа ОС свой край, прямоугольник под ним читается как рамка.
 */
export type DownloadFileIcon = { url: string; kind: 'thumb' | 'shell' };
