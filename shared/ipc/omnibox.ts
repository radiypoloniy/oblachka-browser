...
export interface SuggestDropdownItem {
  ...
  windowId?: number;
  /**
   * Страница УЖЕ ОТКРЫТА в этой вкладке — но открывать по подсказке всё равно будем дублем.
   *
   * ⚠️ Раньше такая строка приходила с `kind: 'tab'`, и Enter уводил на старую вкладку. Человек,
   * набравший адрес или имя сайта, чаще хочет открыть страницу, а не телепортироваться к своей
   * прошлой (живая жалоба и та, что была до неё — «хочу дубль»). Теперь основное действие всегда
   * одно и предсказуемое — открыть; а переход остаётся отдельной кликабельной пометкой в строке.
   *
   * ⚠️ Не путать с `kind: 'tab'`: он остался у СМЫСЛОВОГО поиска вкладок, где человек ищет именно
   * вкладку, и переключение там — то, за чем пришли.
   */
  openTab?: { tabId: string; windowId?: number };
...
// Ссылка внутри скопированного куска: подпись, какой её видел человек, и абсолютный адрес.
// ⚠️ Адрес ВСЕГДА абсолютный: относительный href теряет смысл, как только запись переживает
// страницу-источник, а буфер именно для этого и нужен.
...
export interface ClipboardEntry {
  ...
  title: string;
  at: number;
  // Разметка скопированного куска — чтобы повторная копия ИЗ БУФЕРА не теряла ссылки. При обычном
  // копировании со страницы Chromium кладёт text/html в буфер ОС сам, а вот наш `clipboard:put`
  // раньше делал только writeText: вставленное из списка приезжало голым текстом (живая жалоба
  // «скопировал ссылку — вставился один текст»). Пусто, когда копировали из поля ввода или когда
  // разметка не влезла в предел.
  html?: string;
  // Ссылки из того же куска, разобранные отдельно, — чтобы поповер мог их показать и дать
  // скопировать сам адрес, не разбирая html в рендерере.
  links?: ClipboardLink[];
  // Закреплено человеком: идёт первым в списке, не вытесняется пределом и ПЕРЕЖИВАЕТ перезапуск
  // (зашифрованный файл, см. electron/ClipboardPins.ts). Единственное, что вообще попадает на
  // диск из буфера, — остальное по-прежнему живёт только в памяти и только на сеанс.
  pinned?: boolean;
}
