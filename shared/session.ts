...
export interface SavedTab {
  url: string;
  /**
   * Профиль, которому принадлежит вкладка (см. shared/profiles.ts).
   *
   * ⚠️ НЕОБЯЗАТЕЛЬНОЕ и добавлено БЕЗ смены версии формата — ровно как title/faviconData до него.
   * Файлы, записанные до профилей, поля не имеют, и читающий код обязан считать такие вкладки
   * основным профилем: их куки и логины лежат именно там. Ломать формат ради одного поля значило
   * бы рисковать вкладками людей, а это худшая из возможных цен (см. CLAUDE.md).
   */
  profileId?: string;
