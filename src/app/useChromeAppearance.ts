...
import { buildChromeGround, buildChromeGroundFromMesh, accentFromMesh } from '../../shared/chromeGround';
...
export function useChromeAppearance(incognito: boolean): { ground: Ground | null; dark: boolean } {
  ...
  // ⚠️ ЗДЕСЬ ЖИЛ ЭФФЕКТ, КРАСИВШИЙ ПОЛОСУ СИСТЕМНЫХ КНОПОК, и его больше нет — кнопки рисует сам
  // хром (см. toolbar/WindowControls.tsx). Уходит вместе с ним и весь его аппарат: разрешение
  // цвета пробным элементом (значение стало color-mix, регулярка на hex перестала совпадать),
  // пять попыток по кадрам (первый кадр приходил раньше, чем применялись data-theme и палитра),
  // фолбэк по теме (иначе при осечке светлая полоса поверх тёмного окна) и отдельная верхняя
  // ступень градиента ради полосы. Ни одна из этих подпорок не была лишней — они лечили то, что
  // ОС красила прямоугольник ОДНИМ СПЛОШНЫМ ЦВЕТОМ вне нашей раскладки. Теперь лечить нечего.
