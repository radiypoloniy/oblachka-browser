          ...
          <div ref={omniboxPillRef} style={{
            ...
            <input
              ...
              onMouseDown={() => {
                ...
                // ⚠️ Тронули ТЕКСТ — подсветка строки в дропдауне гаснет. Иначе на экране две
                // «выделенных» вещи сразу: строка выдачи и текст, который человек выделяет мышью, —
                // и жалоба звучала ровно так: «выделяется не то, что в адресной строке, а первый
                // элемент в выдаче». Владелец выделения ровно один, и в момент работы мышью по
                // тексту это адресная строка. Заодно Enter после этого уходит по набранному, а не
                // на подсказку, которую человек уже перестал выбирать.
                if (selectedIdx !== -1) {
                  setSelectedIdx(-1);
                  void window.oblako.setSuggestDropdownHighlight(-1);
                }
