...
const WIDGET_CHOICES: { key: string; label: string; hint: string; size: keyof typeof WIDGET_SIZES }[] = [
  ...
  // ⚠️ Про сеть здесь сказано ЧЕСТНО и своей формулировкой: сам виджет никуда не ходит и
  // ничего не сообщает, но обложку он показывает по ссылке сервиса — то есть картинка
  // грузится с того же сайта, который человек и так слушает.
  { key: 'music',    label: 'Музыка',           hint: 'Управление тем, что играет · обложка грузится с сервиса', size: 'medium' },
