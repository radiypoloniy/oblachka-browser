...
export function CalendarFace({ now }: { now: Date }) {
  ...
  // ⚠️ Высоту ПЛИТКИ меряем отдельно от высоты сетки, и это не лишний ResizeObserver.
  // Решение «показывать ли год» раньше принималось по высоте СЕТКИ — а сетка сама зависит от
  // этого решения: без года ей достаётся больше места, с годом меньше. Получался замкнутый круг,
  // и на плитке высотой 260…300 px он не сходился: год то появлялся, то исчезал сотни раз в
  // секунду. Это и есть «мерцание календаря» (замер: 2000+ перерисовок за 0,7 с внутри полосы,
  // ровно ноль вне её). Полоса совпадает с плиткой в два ряда на крупном масштабе (2×140+14=294)
  // и на среднем при широком окне, где зазор дорастает до 20–24 (2×120+24=264); на мелком
  // масштабе плитка не превышает 248, поэтому там мерцания и не видно.
  // Плитка своего размера от содержимого не берёт — на ней круг размыкается.
  const [shell, shellBox] = useBoxSize<HTMLDivElement>();
  ...
  //
  // ⚠️ Порог задан по ПЛИТКЕ, хотя смысл его — в сетке («датам нельзя ниже 170 px»). Перевод
  // измеренный, а не придуманный: в раскладке С ГОДОМ сетка получает примерно 0,75 высоты плитки
  // минус 57 px, то есть до 170 она дотягивает начиная с 303. Считать это выражение в коде нельзя
  // — оно и есть тот самый круг; поэтому здесь одно число и объяснение, откуда оно.
  const YEAR_MIN_TILE_H = 304;
  const tiny = shellBox.h > 0 && shellBox.h < YEAR_MIN_TILE_H;
  return (
    <div ref={shell} style={tileShell({ padding: pad(4), gap: sp(2) })}>
