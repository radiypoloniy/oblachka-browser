 ...
 *
 * ⚠️ Предпросмотр — тот же html, который сохранится и откроется вкладкой. Один рендерер на
 * стиль, иначе человек сохраняет не то, что видел; здесь это вдвойне обязательно — тело писала
 * модель, и второй его реализации быть не может в принципе.
 *
 * ⚠️ sandbox="allow-scripts", а НЕ пустой, и это осознанное послабление с разбором.
 * Пустой sandbox запрещал скрипты полностью — правильное решение для чужого текста, но оно же
 * запрещало ЛЮБУЮ интерактивность: ни кнопок, ни раскрывающихся блоков, ни клавиш в
 * презентации. Ровно поэтому два стиля из трёх вышли безжизненными.
 *
 * ⚠️ Что осталось закрытым. Без `allow-same-origin` страница получает НЕПРОЗРАЧНЫЙ origin:
 * доступа к родителю, к нашим куки и к localStorage у неё нет, `parent`/`top` для неё чужие.
 * Скрипты в страницу кладём МЫ (см. STYLE_JS в page/styles.ts) — ответ модели по-прежнему
 * проходит через sanitizeDocHtml, который выбрасывает её `<script>` вместе с содержимым.
 * То есть исполняется только наш код, и только в песочнице без прав.
 */
export function PageView({ json }: { json: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, height: '100%' }}>
      ...
      <iframe
        title={page.title} srcDoc={html} sandbox="allow-scripts"
