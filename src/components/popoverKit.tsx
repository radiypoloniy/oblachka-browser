...
export function PopoverCard({ width = 280, invert = false, tail, children }: {
   ...
   *
   * ⚠️ Право на неё имеет ОДИН КЛАСС карточек — «вопрос, требующий ответа сейчас». Причина в
   ...
   *
   * ⚠️ Членов у класса ТРИ, и все — вопрос, требующий ответа сейчас: сайт просит доступ
   * (PermissionPrompt), внешняя программа просит изменить браузер (src/mcpprompt.tsx), браузер
   * просит поставить обновление (UpdatePrompt). Третий добавлен осознанно: речь у них одна, и
   * рисовать вопрос тремя разными карточками — значит учить человека, что вопросы бывают важные
   * и не очень. ⚠️ Четвёртому не давать: инверсия работает потому, что за ней стоит ровно
   * одно значение, а не потому, что она красивая.
   */
  invert?: boolean;
