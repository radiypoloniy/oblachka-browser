...
export function PopoverCard({ width = 280, invert = false, tail, children }: {
   ...
   *
   * ⚠️ Членов у класса ДВА, и оба — разрешение: сайт просит доступ (PermissionPrompt) и внешняя
   * программа просит изменить браузер (src/mcpprompt.tsx). Второй добавлен осознанно: речь у них
   * одна, и рисовать вопрос двумя разными карточками — значит учить человека, что вопросы бывают
   * важные и не очень. ⚠️ Третьему не давать: инверсия работает потому, что за ней стоит ровно
