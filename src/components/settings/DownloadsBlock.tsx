  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <label style={{
        ...
        <span style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <span style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-strong)' }}>
            {t('Спрашивать, куда сохранять каждый файл')}
          </span>
          <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-faint)' }}>
            {t('Выключено — файлы сразу попадают в папку «Загрузки», а прогресс виден в панели загрузок.')}
          </span>
        </span>
      </label>
      ...
      <StatusCard
        ...
        title={t('Скачанные файлы помечаются как пришедшие из интернета')}
        subtitle={t('Windows проверяет такие файлы при запуске, а документы Office открывает в защищённом просмотре. Программы и скрипты браузер спрашивает отдельно — даже когда выбор папки выключен.')}
      />
    </div>
  );
}
