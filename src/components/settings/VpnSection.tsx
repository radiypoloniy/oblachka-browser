...
export default function VpnSection() {
  const { language, t } = useLanguage(); const [status, setStatus] = useState<VpnStatus | null>(null);
  ...
  async function handleSave() {
    ...
    if (!url) { setError(t('Введите ссылку подписки')); return; }
    setSaving(true); setError(''); setInfo('');
    const res = await window.oblako.setVpnSubscription(url);
    setSaving(false);
    if (res.ok) {
      setUrlInput('');
      setInfo(t('Загружено серверов: {n}', { n: res.count ?? 0 }) + (res.skipped ? t(', не распознано: {n}', { n: res.skipped }) : ''));
    } else {
      setError(res.error ?? t('Не удалось сохранить'));
    }
  }
  ...
  async function handleRefresh() {
    ...
    setRefreshing(false);
    if (res.ok) setInfo(t('Обновлено. Серверов: {n}', { n: res.count ?? 0 }) + (res.skipped ? t(', не распознано: {n}', { n: res.skipped }) : ''));
    else setError(res.error ?? t('Не удалось обновить'));
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: sp(6) }}>
      ...
      <SectionHeader
        title="VPN"
        hero={connState === 'running' ? (conn?.serverRemark ?? t('Подключено')) : t(VPN_STATE_WORD[connState])}
        heroLabel={connState === 'running'
          ? t('трафик браузера идёт через сервер')
          : connState === 'error'
            ? (conn?.error ?? t('туннель не поднялся'))
            : t('трафик идёт напрямую')}
      >
        {t('Ссылка подписки и серверы хранятся зашифрованными на этом устройстве и никуда, кроме вашего провайдера, не отправляются.')}
      </SectionHeader>
      ...
      <FactGrid>
        <Fact
          label="Kill switch"
          hint={t('Туннель упал — сеть закрывается')}
          value={connState === 'running' ? t('Сторожит') : t('В покое')}
          active={connState === 'running'}
        />
        <Fact
          label="WebRTC"
          hint={t('Локальный адрес не утекает')}
          value={connState === 'running' ? t('Закрыт') : t('Как обычно')}
          active={connState === 'running'}
        />
        <Fact
          label={t('Серверы')}
          hint={status.fetchedAt ? t('обновлено {date}', { date: new Date(status.fetchedAt).toLocaleDateString(language === 'en' ? 'en' : 'ru') }) : t('подписка не добавлена')}
          value={status.hasSubscription ? String(status.serverCount) : t('Нет')}
        />
        <Fact
          label={t('Протокол')}
          hint={t('Xray-core дочерним процессом')}
          value="VLESS"
        />
      </FactGrid>
      ...
      <StatusCard
        ...
        title={status.hasSubscription ? t('Подписка сохранена') : t('Подписка не добавлена')}
        ...
        subtitle={status.hasSubscription
          ? [t('серверов: {n}', { n: status.serverCount }), status.fetchedAt ? t('обновлено {date}', { date: new Date(status.fetchedAt).toLocaleString(language === 'en' ? 'en' : 'ru') }) : null]
            .filter(Boolean).join(' · ')
          : undefined}
        actions={status.hasSubscription && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            >
              <RefreshCw size={14} /> {refreshing ? t('Обновление…') : t('Обновить')}
            </button>
      ...
      <div>
        <CapsLabel>{t('Ссылка подписки')}</CapsLabel>
        <InputRow>
          <TextField
            value={urlInput}
      ...
      {servers.length > 0 && (
        <div>
          <CapsLabel>{t('Серверы ({n})', { n: servers.length })}</CapsLabel>
          <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-faint)', margin: '0 0 8px' }}>
            {t('Нажмите на сервер, чтобы подключиться.')}
          </div>
          <OptionList>
            {servers.map((s) => {
              ...
              return (
                <OptionRow
                  ...
                  badge={
                    isRunning ? { text: t('подключён'), color: 'var(--success-500)' }
                      : isStarting ? { text: t('подключаюсь…'), color: 'var(--text-muted)' }
                        : isError ? { text: t('ошибка'), color: 'var(--danger-500)' }
                          : undefined
                  }
                  actions={(isRunning || isError) && (
                    <>
                      ...
                      {isError && (
                        ...
                        >
                          {isStarting ? t('Подключение…') : t('Повторить')}
                        </button>
                      )}
