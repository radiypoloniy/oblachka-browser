...
/**
 * Фактура ОБЫЧНОЙ земли — там, где цветной подкраски нет.
 *
 * ⚠️ Зерно в проекте было, но доставалось только цветному фону (chromeTintStyle выше) и виджетам
 * стола, то есть ровно тем местам, где площадь маленькая. Большая ровная заливка на всё окно
 * читается пластиком: в природе идеально ровных поверхностей нет, и глаз это знает. Тот же приём
 * держат Arc, Family и Rive — зерно и едва заметный вертикальный градиент на крупных плоскостях.
 *
 * ⚠️ Градиент считается ОТ ТОКЕНОВ, а не задаётся цветом: доля поверхности, растворяющаяся к
 * низу. Поэтому он верен во всех шести палитрах и в обеих темах — светлеет сверху там, где земля
 * светлая, и там, где тёмная.
 */
export const GRAIN_ENABLED = true;
export function chromeGrainStyle(): React.CSSProperties {
  if (!GRAIN_ENABLED) return {};
  return {
    backgroundImage:
      `${NOISE_SVG}, linear-gradient(180deg, color-mix(in srgb, var(--surface) 26%, transparent) 0%, transparent 62%)`,
    backgroundRepeat: 'repeat, no-repeat',
    backgroundSize: '180px 180px, 100% 100%',
  };
}
