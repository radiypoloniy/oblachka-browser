...
// ⚠️ Ось строго ВЕРТИКАЛЬНАЯ (180deg), и это не вкусовщина. Полосу системных кнопок Windows
// рисует ОС отдельным цветом (setTitleBarOverlay) — она обязана совпасть с тем, что нарисовано
// под ней. У диагональной оси (было 160deg) цвет в правом верхнем углу уже отличается от первой
// ступени: замерено #DEE6F6 против посылаемого #D5E4F8, и полоса читалась чужой заплаткой.
// При вертикальной оси цвет постоянен вдоль каждой горизонтали, поэтому верхняя кромка ТОЧНО
// равна первой ступени — её и отдаём в титлбар (см. App.tsx).
export const CHROME_TINT_TOP = 'color-mix(in srgb, var(--sidebar-tint) 12%, var(--app-bg))';

export const chromeTint: React.CSSProperties = {
  backgroundImage:
    `${NOISE_SVG}, linear-gradient(180deg,` +
    ` ${CHROME_TINT_TOP} 0%,` +
    ' color-mix(in srgb, var(--sidebar-tint) 7%, var(--app-bg)) 45%,' +
    ' color-mix(in srgb, var(--sidebar-tint) 3%, var(--app-bg)) 100%)',
  backgroundRepeat: 'repeat, no-repeat',
  backgroundSize: '120px 120px, 100% 100%',
};

// Фон ВЫДЕЛЕННОГО элемента (активная вкладка, активный сегмент переключателя, выбранная папка) при
// цветном фоне. Ставится переменной, а не флагом в пропах: мест много и лежат они в разных
// компонентах, переменная наследуется вниз сама.
//
// ⚠️ Тот же материал, что у остальных островов, а НЕ «тон погуще». Было 16% тона по белому — но
// земля в верхней части градиента это 12% того же тона по --app-bg, и получались практически
// ОДИН И ТОТ ЖЕ цвет: активная вкладка пропадала ровно там, где градиент насыщеннее всего.
// Выделение здесь держится не насыщенностью, а тем же, чем и всё парящее: светлее земли + тень.
export const TINTED_PLATE_VAR = 'color-mix(in srgb, var(--sidebar-tint) 6%, var(--surface))';
...
export const TINTED_PLATE_VARS: React.CSSProperties = {
  ['--plate-bg' as string]: 'color-mix(in srgb, var(--sidebar-tint) 6%, var(--surface))',
