 ...
 */
export const GRAIN = { light: 0.09, dark: 0.09, tinted: 0.075, material: 0.06, overlay: 0.09 } as const;
...
/**
 * Поверхность карточки, которая живёт В СВОЕЙ ВЬЮ НАД СТРАНИЦЕЙ: поповеры, панель поиска,
 * выпадашка омнибокса.
 *
 * ⚠️ НЕПРОЗРАЧНАЯ — единственная честная реализация материала на этом слое. `backdrop-filter`
 * сквозь границу WebContentsView не работает (разбор у --overlay-plate в tokens/colors.css), а
 * полупрозрачность без размытия означает, что текст страницы просвечивает рябью. Здесь и
 * `backdrop-filter` не задаётся вовсе: он был бы чистым расходом GPU без единого пикселя эффекта.
 *
 * ⚠️ ЗЕРНО СИЛЬНЕЕ, ЧЕМ У МАТЕРИАЛА (0.09 против 0.06), и прежнее рассуждение «на маленькой
 * площади тот же шум был бы мусором» здесь не действует: там неровность поверхности читалась в том
 * числе через просвечивающую подложку, а тут её не осталось — фактуру держит только зерно.
 * Ровная непрозрачная заливка на площади карточки читается пластиком.
 */
export const overlayPlate: React.CSSProperties = {
  backgroundImage: noise(GRAIN.overlay),
  backgroundColor: 'var(--overlay-plate)',
